...
// Features: Top 24h Price Increase
// ============================================
...
export function GainersWidget({ width = 200, height = 200 }: { width?: number; height?: number }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('/api/coingecko/gainers');
    ...
    return (
        <div style={{ ...baseStyle }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                <div>
                    <div style={{ fontSize: isLarge ? '16px' : '12px', fontWeight: 'bold', color: THEME.green }}>TOP GAINERS</div>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {data.slice(0, displayCount).map((item, i) => (
                        <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <div style={{ fontSize: '10px', color: THEME.green, fontWeight: 'bold' }}>
